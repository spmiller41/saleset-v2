...
public class TwilioManager {
    ...
    @Value("${webhook-relay.endpoint.url}")
    private String webhookRelayURL;

    @Value("${twilio.outbound.call.handler}")
    private String outboundCallHandler;
    ...
    public PhoneNumberDetails lookupPhoneNumber(String phoneNumber) {
        try {
            ...
            if (!number.getValid() || number.getLineTypeIntelligence() == null ||
                    number.getLineTypeIntelligence().get("type") == null) return null;
            ...
            if (twilioApiException.getStatusCode() == 404) return null;
